...
<template>
  <header class="flex justify-between	items-center w-full mt-8 mb-9 md:mt-32">
    ...
    <div class="flex space-x-4 group text-gray-500 hover:text-cyan-blue-950 dark:hover:text-gray-500 dark:text-white">
      <p class="text-xs font-bold">{{ theme === "dark" ? "LIGHT" : "DARK"}}</p>
      <button
        type="button"
        @click="toggleTheme"
      >
        <svg v-if="theme === 'dark'" width="20" height="20" xmlns="http://www.w3.org/2000/svg">
          <g data-testid="icon" class="fill-white group-hover:fill-gray-500" fill-rule="nonzero">
        ...
        <svg data-testid="icon" v-else class="fill-gray-500 group-hover:fill-cyan-blue-950" width="20" height="20" xmlns="http://www.w3.org/2000/svg">
